...
int main()
{
    ...
    // * strcpy returns the first argument
    lPtr = strcpy(lString, LEARNING_PATH);

    // #10 - strncpy - A better way to copy string
    // - Can result in a non-terminated stirng!
    lPtr = strncpy(lShortAnswer, "abc", sizeof(lShortAnswer));

    // Programmer should use strncpy this way!
    memset(&lShortAnswer, 0, sizeof(lShortAnswer));
    lPtr = strncpy(lShortAnswer, "abc", sizeof(lShortAnswer) - 1);

    // #11 - strcat - Concaten strings
    lPtr = strcat(lString, " - ");
    lPtr = strcat(lString, COURSE);
